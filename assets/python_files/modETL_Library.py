...
# added:
#
# funcReadWorkingFilesReturnDictionary - reads csv file(s) from WorkingFiles and returns a dictionary of files found
# funcReadCleanedFilesReturnDictionary - reads csv file(s) from CleanedFiles and returns a dictionary of files found
# funcReadOriginalFilesReturnDictionary - reads csv file(s) from OriginalFiles and returns a dictionary of files found
# funcReadOriginalFilesReturnDataFrame - reads a specific csv file from the OriginalFiles folder and returns a DataFrame of the data
#
...
#                                 dataframes with the  KEY as the filename and the VALUE as the dataframe 
#
# funcReadOriginalFilesReturnDictionary - reads csv file(s) from OriginalFiles  and returns a dictionary of 
#                                 dataframes with the  KEY as the filename and the VALUE as the dataframe 
#
...
def funcReadOriginalFilesReturnDictionary():
    """
    Created 15/07/2026 By Roger Williams
    
    Reads csv file(s) from OriginalFiles and returns a dictionary of dataframes with the
    ...
    #get list of files in OriginalFiles folder
    lstFiles = os.listdir(os.getcwd() + modGlobal.CNST_STR_DATA_ORIGINALPATH)
    ...
    for strFile in lstFiles:
        ...
        if strFile.endswith(".csv"):
            ...
            dfTemp = pd.read_csv(os.getcwd() + modGlobal.CNST_STR_DATA_ORIGINALPATH + "/" + strFile)
            #name DataFrame
...
def funcGetStructure(dfWhat : pd.DataFrame):
    ...
    dfStrings = dfWhat.select_dtypes(include=["object"])    # "str","category"
...
def funcGetCategoricalValueDistribution(dfWhat : pd.DataFrame):
    ...
    for colColumn in dfWhat.select_dtypes(include=["object"]).columns:
...
def funcGetColumnSkew(dfWhat : pd.DataFrame):
    ...
    for colColumn, objValue in dfWhat.items():
        ...
        if not objValue.dtype.name == "object":
